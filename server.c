...
int main(int argc, char *argv[])
{
	...
	for (i=0;i<4;i++)
	{
     ...
     {    
        ...
        if (fsmServer==0)
        {
        	switch (buffer[0])
        	{
                	case 'C':			// (2)acceptation de la connexion					
				...
				sleep(1); //ceci est un commentaire
	...
	else if (fsmServer==1)
	{
		switch (buffer[0])
		{
				...
				sscanf(buffer,"G %d %d",&gId ,&guilt);
				if (guilt == deck[12]){
					printf("Le joueur %d est bon\n",gId);
				}
				else{printf("Le joueur %d est mauvais\n",gId);}
				sprintf(reply,"M %d",-1);
				broadcastMessage(reply);
				break;
                	case 'O':
				sscanf(buffer,"O %d %d",&gId ,&obj);
				
				for(i=0; i<4; i++){		//On envoit au joueur courant la valeur 100 si un joueur possède le symbole désiré
					if(i!=gId){
						if(tableCartes[i][obj]!=0){
							sprintf(reply,"V %d %d %d",i,obj,100);
							for(j=0;j<4;j++){
								if(j!=i){							
			...
			case 'S':
				...
				sscanf(buffer,"S %d %d %d",&gId,&perso,&obj);
				sprintf(reply,"V %d %d %d",perso,obj,tableCartes[perso][obj]); 
				broadcastMessage(reply);
				sprintf(reply,"M %d",joueurCourant++);
				broadcastMessage(reply);
				// RAJOUTER DU CODE ICI
