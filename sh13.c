...
int main(int argc, char ** argv)      //ip address serv, port serv, ip address cli, port cli, nom 
{
	int ret;
	int i,j;
	int indj,indo,val;	
    ...
    SDL_Init(SDL_INIT_VIDEO); //initialise la bibliotheque graphique SDL
	TTF_Init();//initialise la bibliotheque SDL pour les polices de caracteres TTF
 
    SDL_Window * window = SDL_CreateWindow("Sherlock 13 - MAHTANI & PESAMOSCA",SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 1024, 768, 0);
 	//creer la fenetre du jeu, 1024 pixels de large sur 768 pixels de haut
    SDL_Renderer *renderer = SDL_CreateRenderer(window, -1, 0);
    //association du renderer a la fenetre
    SDL_Surface *deck[13],*objet[8],*gobutton,*connectbutton;
    //declaration des pointeurs sur surfaces

	deck[0] = IMG_Load("SH13_0.png"); //on charge les images dans les surfaces créées
	...
	//variables qui determinent l'action du joueur courant
	joueurSel=-1; //joueur
	objetSel=-1;  //symbole
	guiltSel=-1;  //personnage suspect
    ...
    SDL_Texture *texture_deck[13],*texture_gobutton,*texture_connectbutton,*texture_objet[8]; //declaration de pointeurs sur textures

	for (i=0;i<13;i++)
		texture_deck[i] = SDL_CreateTextureFromSurface(renderer, deck[i]); //on affecte les pointeurs avec les textures creees a partir des surfaces
	for (i=0;i<8;i++)
    ...
    TTF_Font* Sans = TTF_OpenFont("sans.ttf", 15); // on charge la police de caracteres que l'on va utiliser pour afficher du texte
    printf("Sans=%p\n",Sans); //test pour la police de caractere

   // Creation du thread en mode serveur tcp
   printf ("Creation du thread serveur tcp !\n");
   synchro=0;
   ret = pthread_create ( & thread_serveur_tcp_id, NULL, fn_serveur_tcp, NULL);

	// Boucle evenementielle: recuperation des evenements
    while (!quit)
    {
	if (SDL_PollEvent(&event))
	{
		//printf("un event\n");
        	switch (event.type)
        	{
            		case SDL_QUIT://fermeture du la fenetre
                		quit = 1;
                		break;
			case  SDL_MOUSEBUTTONDOWN:      //lors d'un clic de la souris
				SDL_GetMouseState( &mx, &my );//on récupere les coordonnées actuelles
				//printf("mx=%d my=%d\n",mx,my);
				if ((mx<200) && (my<50) && (connectEnabled==1)) // connexion
				{
					sprintf(sendBuffer,"C %s %d %s",gClientIpAddress,gClientPort,gName);
					sendMessageToServer("localhost", 32000,sendBuffer);
					// RAJOUTER DU CODE ICI <<

					connectEnabled=0;
				}
				else if ((mx>=0) && (mx<200) && (my>=90) && (my<330)) //lorsqu'on clique sur une case joueur
				{
					joueurSel=(my-90)/60;
					guiltSel=-1;
				}
				else if ((mx>=200) && (mx<680) && (my>=0) && (my<90)) //clic sur une case symbole
				{
					objetSel=(mx-200)/60;
					guiltSel=-1;
				}
				else if ((mx>=100) && (mx<250) && (my>=350) && (my<740)) //clic sur une case personnage que l'on suspecte
				{
					joueurSel=-1;
					objetSel=-1;
					guiltSel=(my-350)/30;
				...
				else if ((mx>=500) && (mx<700) && (my>=350) && (my<450) && (goEnabled==1))
				{
					...
					if (guiltSel!=-1)  //on accuse 
					{
						sprintf(sendBuffer,"G %d %d",gId, guiltSel);
						sendMessageToServer("localhost", 32000,sendBuffer);

					}
					else if ((objetSel!=-1) && (joueurSel==-1)) //on demande qui en a
					{
					...
					else if ((objetSel!=-1) && (joueurSel!=-1))//on demande combien 
					{
				...
				{
					joueurSel=-1; //on reinitialise pour le prochain joueur
					objetSel=-1;
					guiltSel=-1;
				}
				break;
		...
		switch (gbuffer[0])
		{
			...
			case 'V':
				sscanf(gbuffer,"V %d %d %d",&indj,&indo,&val);
				tableCartes[indj][indo] = val;
				break;
		}
		synchro=0;
                pthread_mutex_unlock( &mutex );
	...
	//Interface graphique : on affiche un fond blanc
	...
	{// Puis on affiche les 8 objets. Les SDL_Rect definissent ou on affiche le bitmap
        ...
        SDL_RenderCopy(renderer, texture_objet[1], NULL, &dstrect_ampoulse);
		...
		// Puis on affiche le nombre associe a ces 8 objets
	...
	for (i=0;i<4;i++)
  	...
  	SDL_DestroyTexture(texture_deck[0]); //liberation des textures
    SDL_DestroyTexture(texture_deck[1]);
    SDL_DestroyTexture(texture_deck[2]);
    SDL_DestroyTexture(texture_deck[3]);
    SDL_DestroyTexture(texture_deck[4]);
    SDL_DestroyTexture(texture_deck[5]);
    SDL_DestroyTexture(texture_deck[6]);
    SDL_DestroyTexture(texture_deck[7]);
    SDL_DestroyTexture(texture_deck[8]);
    SDL_DestroyTexture(texture_deck[9]);
    SDL_DestroyTexture(texture_deck[10]);
    SDL_DestroyTexture(texture_deck[11]);
    SDL_DestroyTexture(texture_deck[12]);
    
    SDL_FreeSurface(deck[0]); //liberation des surfaces deck
    SDL_FreeSurface(deck[1]);
    SDL_FreeSurface(deck[2]);
    SDL_FreeSurface(deck[3]);
    SDL_FreeSurface(deck[4]);
    SDL_FreeSurface(deck[5]);
    SDL_FreeSurface(deck[6]);
    SDL_FreeSurface(deck[7]);
    SDL_FreeSurface(deck[8]);
    SDL_FreeSurface(deck[9]);
    SDL_FreeSurface(deck[10]);
    SDL_FreeSurface(deck[11]);
    SDL_FreeSurface(deck[12]);
    
    SDL_FreeSurface(objet[0]); //liberation des surfaces objet
    SDL_FreeSurface(objet[1]);
    SDL_FreeSurface(objet[2]);
    SDL_FreeSurface(objet[3]);
    SDL_FreeSurface(objet[4]);
    SDL_FreeSurface(objet[5]);
    SDL_FreeSurface(objet[6]);
    SDL_FreeSurface(objet[7]);
    SDL_FreeSurface(objet[8]);
    SDL_FreeSurface(objet[9]);
    SDL_FreeSurface(objet[10]);
